...
export default function DashboardUsuario() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await api.post("/chamados", { titulo, descricao, prioridade, categoria });
    ...
    return (
        <>
            ...
            <div className="dashboard-container">
                    ...
                    {chamados.length > 0 ? (
                        chamados.map((c) => (
                            <div key={c.id} className="chamado-card">
                                ...
                                <p>{c.descricao}</p>
                                <div className="tags">
                                    <span className="status" style={{ backgroundColor: getStatusColor(c.status) }}> {statusLabels[c.status] || c.status}</span>
                                    <span className="prioridade" style={{ backgroundColor: getPrioridadeColor(c.prioridade) }}>{c.prioridade}</span>
                ...
                <button className="btn-new" onClick={() => setShowModal(true)}>+</button>
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="modal-actions">
                                    <button type="submit" className="btn-submit" disabled={loadingCreate}>
                                        {loadingCreate ? <div className="loader"></div> : "Criar"}
                                    </button>
        ...
        * { font-family: 'Poppins', sans-serif; }
        ...
        .status { background-color: #f39c12; }
        .prioridade { background-color: #2ecc71; }
        .categoria { background-color: #8e44ad; }
        ...
        .btn-delete { position: absolute; top: 15px; right: 15px; background-color: #e74c3c; color: #fff; border: none; padding: 5px 10px; border-radius: 5px; cursor: pointer; font-size: 12px; }
        ...
        .modal input, .modal textarea, .modal select { width: 100%; padding: 10px; margin: 8px 0; border: 1px solid #ccc; border-radius: 5px; font-size: 14px; box-sizing: border-box; }
        .modal textarea { resize: none; height: 80px; }
        .modal-actions { display: flex; justify-content: flex-end; margin-top: 15px; }
        .btn-submit { background-color: #2ecc71; color: #fff; border: none; padding: 8px 16px; border-radius: 5px; cursor: pointer; display: flex; justify-content: center; align-items: center; }
        .btn-submit:hover { background-color: #27ae60; }
        .btn-cancel { background-color: #e74c3c; color: #fff; border: none; padding: 8px 16px; border-radius: 5px; cursor: pointer; margin-left: 10px; }
        .btn-cancel:hover { background-color: #c0392b; }
        .popup { position: fixed; top: 20px; right: 20px; padding: 12px 20px; border-radius: 5px; color: #fff; font-weight: 500; box-shadow: 0 4px 8px rgba(0,0,0,0.2); z-index: 2000; opacity: 0.95; animation: fadeInOut 3s forwards; }
        ...
        .popup-confirm { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: #fff; padding: 20px 30px; border-radius: 10px; box-shadow: 0 5px 15px rgba(0,0,0,0.3); z-index: 3000; text-align: center; }
        .confirm-actions { margin-top: 15px; display: flex; justify-content: center; gap: 15px; }
        .loader { border: 4px solid #f3f3f3; border-top: 4px solid #6a11cb; border-radius: 50%; width: 18px; height: 18px; animation: spin 1s linear infinite; margin: auto; }
